...
public class MainCharacter : Actor 
{
	...
	public static MainCharacter Instance => instance;
	...
	public override void Update ()
	{
		...
		if (firstA)
			aadder += Time.deltaTime;
		if (firstD)
			dadder += Time.deltaTime;
		
		
		//Debug.Log("_XSpeed: "+_playerVelocityX+  "  _YSpeed: "+_playerVelocityY);
	}
	...
	private void Jump()
	{
		...
		if (_stateMachine.State == PlayerStates.InAir && _canJump > 0)
		{
//			Debug.Log("DOUBLE JUMP");
	...
	private void GetPlayerInput()
	{
		if (_inControl)
		{
			...
			{
				AudioMgr.Instance.PlayAuEffect("Jump");
				OnJumpKeyUp();
			}
			...
			if (Input.GetKeyDown(KeyCode.LeftShift))
			{
				...
				//print("dashing:  true");
				AudioMgr.Instance.PlayAuEffect("Dash");
				MainLoop.Instance.ExecuteLater(() =>
				{
					_affectedByGravity = true;
					isDashing = false;
	...
	#region StateCallBacks
		...
		private void Idle_Enter()
		{
			...
			_animator.Play(Animator.StringToHash("Idel"));
		}
